...
vector<Quad> optimize(const Thetas& curve, double penalty) {
	vector<Break> breaks;
...
std::string optimize(const std::string& in, double penalty) {
...
#endif
	vector<Quad> optbzs = optimize(thetas, penalty);
